...
/**
 * Login dos membros da área de missões (Supabase Auth, conta por sócio).
 * usuario === undefined → ainda carregando a sessão; null → deslogado.
 */
export function useAuth() {
  ...
  const resetarSenha = useCallback(async (email: string): Promise<string | null> => {
    const e = email.trim();
    if (!e) return "Informe o e-mail para recuperar a senha.";
    const sb = await getSupabase();
    const redirectTo =
      typeof window !== "undefined" ? `${window.location.origin}/missoes` : undefined;
  ...
  return {
    usuario: user,
    nome: user === undefined ? undefined : nomeDe(user),
